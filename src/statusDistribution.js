...
function createStatusDistributionChart(data, filters) {
    ...
    // Área do radar - Modificado para ter apenas a linha
    radarWrapper.append('path')
        .attr('class', 'radar-area')
        .attr('d', d => radarLine(d.values))
        .style('fill', 'none') // Removido o preenchimento
        .style('stroke', d => color(d.category))
        .style('stroke-width', '2px')
        .style('stroke-opacity', 0.8) // Opacidade para a linha
        .on('mouseover', function(event, d) {
            d3.select(this).style('stroke-width', '3px'); // Aumenta a espessura ao passar o mouse
        ...
        .on('mouseout', function() {
            d3.select(this).style('stroke-width', '2px'); // Volta à espessura original
            hideTooltip(tooltip);
        });
